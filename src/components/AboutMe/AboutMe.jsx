import React from 'react'
import './Aboutme.css'

const AboutMe = () => {
  return (
    <>
        <div className="container aboutme" id="aboutme">
          <div className="row">
            <div className="col mx-auto">
              <h2>About Me</h2>
            </div>
          </div>
          <div className="row">
            <div className="col-lg-8 mx-auto">
              <p>Software Developer with experience in Web Development and Data Science.Worked on building web applications, interactive dashboards, and data processing systems.Proficient in Python, Javascript, Frappe, and custom UI/UX integrations.Passionate about creating efficient, user-friendly, and data-driven solutions.Continuously exploring new technologies to improve software development.</p>
              <p>I enjoy solving problems through coding and building software that makes tasks easier. My work involves processing data, improving workflows, and creating useful tools. I focus on writing clean and efficient code to ensure smooth performance. Learning new technologies and improving my skills is something I always strive for.</p>
            </div>
          </div>
        </div>
    </>
  )
}

export default AboutMe